# SmartProps marketing site

The public product/landing page for **smartprops.io** (apex). Static — no build step.
Separate from the app, which lives on **app.smartprops.io** (the Next.js project at the repo root).
...
- `index.html` — the product description (hero, features, how-it-works, pricing→beta, CTA).
  ...
  `https://app.smartprops.io/beta` (the live signup form).
- `_redirects` — Netlify: `www.smartprops.io` → apex `smartprops.io` (301).
...
2. **Domains** → add `smartprops.io` (apex, canonical) and `www.smartprops.io`.
   - DNS: point apex + www at this new Netlify site. Leave the existing `app.smartprops.io`
...
`app.smartprops.io` is auth-gated (everything redirects to `/login`), so it's effectively not
