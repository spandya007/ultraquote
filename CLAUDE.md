...
- **⏭️ FIRST THING NEXT SESSION: run migration 008 in the Supabase SQL editor, then work through `docs/testing-todo-roles.md`.** Quote creation is BROKEN until 008 runs (the `/api/quotes` route now calls the `next_quote_number()` RPC, which 008 creates).
...
- **NEW: Roles & quote ownership BUILT, ⚠️ NOT YET TESTED** (backlog #14, design `docs/roles-permissions-design.md`): creator-owned quotes/templates (owner can always edit; others read-only w/ amber banner + Duplicate), duplicate open to all members, products/settings/client-edits owner-only (clients add-only for members), Extract pricing owner-only, Created-by column on /quotes. Enforced in RLS (migration 008 ⚠️ NOT run) + UI. Quote numbers via atomic `next_quote_number()` RPC. Test plan: **`docs/testing-todo-roles.md`**.
