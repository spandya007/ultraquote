...
## ⏸️ RESUME SNAPSHOT (last session end: 2026-06-10/11)
**Where things stand:**
- **LIVE on Netlify: https://ultraquote.netlify.app** · GitHub `spandya007/ultraquote` `main` (in sync, through `d06b0de`+). PDF service on Railway healthy. **Netlify "Stop builds" is ON** — the live site is running pre-Discount code; everything since (Discount, tenant onboarding, roles/ownership) needs a manual deploy (dashboard: un-stop builds → Trigger deploy).
...
**Backlog (for prioritization):** **NEXT FEATURE = #17 self-serve password reset + change password (user-pinned top priority).** Then: dark mode (#7), product docs polish (#8), BlockNote upgrade + two-column (#10), Withdraw (#12), offline-sign (#13), CSV mapping wizard (#16); tech debt: hex webhook secret, Next major upgrade, Supabase email rate-limit bump, signed-quote content immutability (candidate, not yet backlogged). Tenant onboarding (#11) ✅ shipped + tested; roles/ownership (#14) ✅ built — pending migration 008 + the testing checklist.
...
17. **🔝 TOP PRIORITY — Self-serve password reset + change password** (user-requested 2026-06-11: do this BEFORE any other feature). Two pieces: **(a) "Forgot password?"** link on `/login` → small request form → `supabase.auth.resetPasswordForEmail(email, { redirectTo: <origin>/auth/set-password })` → Supabase recovery email → link lands on the EXISTING `/auth/set-password` page, which already handles implicit-flow `#hash` tokens and password setting (recovery links work exactly like invite links; the welcome copy should adapt — recovery type is in the hash as `type=recovery`). Bare-path redirect only (no query string — allowlist gotcha); the two `/auth/set-password` redirect URLs are ALREADY allowlisted. **(b) "Change password"** for logged-in users (e.g. small form/menu in Settings or the sidebar): `supabase.auth.updateUser({ password })` — no email round-trip needed. Notes: emails go via the configured Zoho SMTP; "Reset Password" email template can be branded like the invite one; rate limits apply. Until built, recovery = Supabase dashboard or admin API.
