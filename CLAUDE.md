...
- `007_platform_admins_and_invites.sql` — `platform_admins` (RLS, NO policies — service-role only) + `tenant_invites` (+ tenant-member read policy) + `provision_tenant_shell()` (ownerless tenant) + seeds Sameer as platform admin. **Required for the /admin console, invite-first tenant onboarding, and Settings → Team.** ⚠️ NOT yet run.
...
**Where things stand:**
- **LIVE on Netlify: https://ultraquote.netlify.app** · GitHub `spandya007/ultraquote` `main` (in sync). PDF service on Railway healthy. **Netlify "Stop builds" is ON** — the live site is running pre-Discount code; the Discount feature (commits through `27c1487`) needs a manual deploy (dashboard: un-stop builds → Trigger deploy).
- **NEW since last snapshot:** line-item **Discount** column — % or fixed $ per line (mutually exclusive selector), all totals/tax/margins on discounted price (floored at 0), client-facing tables show Discount column + green "You save $X" row only when discounts exist (migrations 005+006 ✅ run, verified on CMIT-2026-008); pdf-loader now logs real query errors instead of fake "Quote not found".
