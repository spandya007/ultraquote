...
3. **Environment segregation FIRST, before TOTP** (user raised 2026-06-11): the headline issue is **`.env.local` currently points at the PROD Supabase project (`pibipcdkxtldjbrsdbua`) — so local dev writes to production data** (test tenant Pandya's, test products etc. all live in prod). Plan: spin up a **separate Supabase project for dev/test** (likely free — Supabase free org allows 2 active projects), replay `supabase/migrations/*` + seed, duplicate the one-time config per env (Auth redirect URLs, Zoho SMTP, DocuSeal sandbox webhook), point `.env.local` at dev and Netlify env at prod. Then build **#18 TOTP MFA** against dev (MFA enroll/lockout testing must NOT happen on prod accounts). So sequence: **env split → then TOTP.**
