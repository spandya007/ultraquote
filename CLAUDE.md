...
- `010_add_tenant_contact_name.sql` — `tenants.contact_name` (idempotent `add column if not exists`). schema-drift fix: the column existed on **prod** (manual, pre-migrations) but was never in `schema.sql`, so the fresh **dev** project lacked it and saving Company Settings failed there. ⚠️ **Run on dev** (no-op on prod). Full dev-vs-prod column audit 2026-06-12 confirmed this was the ONLY drift.
