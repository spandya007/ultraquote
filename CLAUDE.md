...
2. **Mirror the password floor server-side in Supabase**: set **Minimum password length = 12**; **leave "Password Requirements" at None** (client is "3 of 4 any", Supabase can't express it — avoids server-stricter rejections, decided 2026-06-11); on **Pro** enable **Leaked Password Protection (HIBP)**. No app code change.
...
5. (When ready) **Netlify deploy** to close the growing gap, then the post-deploy checklist (hex webhook secret · live signing-status-flip test #15 · email rate-limit bump).
