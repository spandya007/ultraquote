...
3. **Environment segregation FIRST, before TOTP** (user raised 2026-06-11): the headline issue is **`.env.local` currently points at the PROD Supabase project (`pibipcdkxtldjbrsdbua`) — so local dev writes to production data** (test tenant Pandya's, test products etc. all live in prod). **▶️ Runbook ready: `docs/dev-environment-setup.md` + `supabase/seed-dev.sql`** — create a free dev Supabase project, run `schema.sql` (full consolidated schema, incl. migrations 001–009), run the seed (DO-block, paste owner UID), add localhost redirect URL, repoint `.env.local` at dev (Netlify keeps prod). Supabase free org = 2 active projects (free; pauses after ~1wk idle). Then build **#18 TOTP MFA** against dev (MFA enroll/lockout testing must NOT happen on prod accounts). Sequence: **env split → then TOTP.**
