...
18. **Two-factor authentication (TOTP / Google Authenticator)** — user-requested 2026-06-11; queued to build next. Supabase built-in MFA, **factor type `totp`** — **free on the Free plan** ("Basic MFA"; verified on supabase.com/pricing — NOT a Pro feature; only Leaked-Password-Protection + "Advanced MFA" enforcement add-ons cost money). Flow: `supabase.auth.mfa.enroll({factorType:'totp'})` → QR + secret → `mfa.challenge()` + `mfa.verify({code})`; sessions carry AAL1/AAL2, can require AAL2 (even in RLS). **Build plan:** Settings → Security section ("Enable 2FA" → show QR → verify 6-digit code → enrolled, with disable + recovery codes), and an MFA challenge step at login when a factor exists. **Decisions still needed:** optional vs mandatory; per-user vs tenant-enforced; include recovery codes (recommended). Email-OTP-as-MFA is NOT a native Supabase factor (only TOTP + phone/SMS) — don't build email-as-2FA. SMS would need a paid Twilio-type provider — skip (prefer existing tools).
