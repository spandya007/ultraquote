...
- `011_mfa_recovery_codes.sql` — `mfa_recovery_codes` (SHA-256 hashes; RLS, NO policies — service-role only). **Required for the TOTP 2FA recovery-codes feature** (Supabase MFA/TOTP itself is native + free, no config). ⚠️ run on dev + prod.
...
## ▶️ NEXT PLAN (updated 2026-06-11 EOD)
1. ~~Test password features~~ ✅ DONE (forgot-password + change-password + policy verified).
2. ~~Mirror the password floor in Supabase~~ ✅ DONE 2026-06-12 on prod + dev: **Minimum password length = 12**, Password Requirements **None** (client is "3 of 4 any"). Leaked Password Protection (HIBP) still pending a Pro upgrade.
3. ~~**Environment segregation**~~ ✅ DONE + verified 2026-06-11. A **separate dev Supabase project** now exists (runbook `docs/dev-environment-setup.md` + `supabase/seed-dev.sql`); `.env.local` repointed at dev, Netlify still on prod (`pibipcdkxtldjbrsdbua`). Local dev no longer writes to prod. **Build #18 TOTP MFA against dev** (MFA enroll/lockout testing must NOT touch prod accounts).
4. **Gemini API**: free tier throws "model overloaded" 503s often — move to **pay-as-you-go** (enable billing on the existing Google AI Studio key; gemini-2.5-flash is cents/month at this volume) + add retry/backoff in `/api/ai/*`. Per-env keys ideal.
5. ~~Netlify deploy + post-deploy checklist~~ ✅ DONE 2026-06-12 (gap closed; hex webhook secret · live signing #15 · email rate-limit bump all done).
6. **NEXT BUILD: #18 TOTP MFA** against the dev project. Needs 3 decisions first: optional vs mandatory · per-user vs tenant-enforced · recovery codes (recommended).
